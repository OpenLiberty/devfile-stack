...
processDevfiles() {
    ...
    # Maven files.
    sed -e "s!{{.STACK_NAME}}!$STACK_NAME!; s!{{.STACK_SHORT_NAME}}!$STACK_SHORT_NAME!; s!{{.LIBERTY_RUNTIME_VERSION}}!$LIBERTY_RUNTIME_VERSION!; s!{{.LIBERTY_MAVEN_PLUGIN_VERSION}}!$LIBERTY_MAVEN_PLUGIN_VERSION!; s!{{.WLP_INSTALL_PATH}}!$WLP_INSTALL_PATH!; s!{{.STACK_IMAGE_MAVEN}}!$STACK_IMAGE_MAVEN!; s!{{.OUTERLOOP_DOCKERFILE_MAVEN_LOC}}!$OUTERLOOP_DOCKERFILE_MAVEN_LOC!; s!{{.OUTERLOOP_DEPLOY_YAML_MAVEN_LOC}}!$OUTERLOOP_DEPLOY_YAML_MAVEN_LOC!" src/devfiles/maven/devfile.yaml > "${destFileRootPath}"/devfiles/maven/devfile.yaml
