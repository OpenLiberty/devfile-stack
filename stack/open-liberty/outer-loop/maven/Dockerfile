...
ARG WLP_INSTALL_PATH=/opt/ol/wlp
ARG LIBERTY_MAVEN_PLUGIN_VERSION=3.5.1

# Step 1: Build the user's application
FROM icr.io/appcafe/open-liberty-devfile-stack:${LIBERTY_VERSION} as compile

ARG LIBERTY_VERSION
ARG WLP_INSTALL_PATH
ARG LIBERTY_MAVEN_PLUGIN_VERSION
...
RUN cd /work/outer-loop-app && \
    ...
    mvn -e -DinstallDirectory=$WLP_INSTALL_PATH io.openliberty.tools:liberty-maven-plugin:${LIBERTY_MAVEN_PLUGIN_VERSION}:create package 
...
RUN cd $WLP_INSTALL_PATH/usr/servers && \
